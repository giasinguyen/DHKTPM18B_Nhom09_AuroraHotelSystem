...
public interface PermissionRepository extends JpaRepository<Permission, String> {
    ...
    @Query("SELECT p FROM Permission p WHERE " +
           "(:name IS NULL OR :name = '' OR CAST(p.name AS string) LIKE CONCAT('%', :name, '%')) AND " +
           "(:description IS NULL OR :description = '' OR CAST(p.description AS string) LIKE CONCAT('%', :description, '%'))")
